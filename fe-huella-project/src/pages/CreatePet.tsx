...
//          */}
//            <label>photo</label>
//           <input type="image" onChange={handleFileChange}   />
...
//             <label>when</label>
//             <input type="date" placeholder="" {...register("when", { required: true })} className="w-500 h-250 p-15 pl-24 pr-302 rounded-12 border border-gray-400" />
...
export const CreatePet: React.FC = () => {
  ...
  const { mutate } = useMutation(
    async (data: CreateFormValues) => {
      ...
      formData.append("type", data.type);
      if (data.photo) {
        formData.append("img", data.photo[0]);
      }
      formData.append("location", data.location);
      formData.append("gender", data.gender);
      formData.append("lostOrFound", data.lostOrFound);
